...
learned_ode_func = nn_templates.L96_ODE_Net_2(x_dim).to(device)
...
lambda1 = lambda2 = lambda epoch: (epoch-9)**(-1) if epoch >=10 else 1
...
for epoch in tqdm(range(50), desc="Training", leave=False):
    ...
    with torch.no_grad():
        ...
        _, _, test_log_likelihood = da_methods.EnKF(learned_ode_func, true_obs_func, t_obs, y_obs_test, N_ensem, init_m, init_C_param, learned_model_Q, noise_R_true, device,
                                                    save_filter_step={}, linear_obs=True, ode_options=dict(step_size=0.01), adjoint_options=dict(step_size=0.05), localization_radius=5, tqdm=None)
