...
for nm in range(n_metaruns):
  ...
  true_ode_func = nn_templates.Linear_ODE_diag(x_dim, true_a).to(device)
  ...
  true_obs_func = nn_templates.Linear(x_dim, y_dim, H_true).to(device)
  ...
  for ne in range(n_epochs):
    ...
    test_ode_func = nn_templates.Linear_ODE_diag(x_dim, test_a).to(device)
    q = 0.5
    ...
    for nr in range(n_runs):
      X, X_track, X_intermediate, neg_log_likelihood = da_methods.EnKF(test_ode_func, true_obs_func, t_obs, y_obs.unsqueeze(-2).repeat(1, 1, 1), N_ensem=N_ensem, init_m=init_m, init_C_param=init_C_param, model_Q_param=test_model_Q, noise_R_param=noise_R_true, device=device, detach_every=None, Cf_track=None, **enkf_kwargs)
    ...
    m_track, C_track, Cf_track, neg_log_likelihood = da_methods.KF(test_ode_func, true_obs_func, t_obs, y_obs, init_m, init_C_param, test_model_Q, noise_R_true, device, **enkf_kwargs)
    neg_log_likelihood.mean().backward()
