from torchEnKF import da_methods, noise
...
def train_loop_em_new(ode_func, obs_func, t_obs, y_obs, out, out_intermediate, N_ensem, init_m, init_C_param, model_Q_param, noise_R_param, device, optimizer, n_epochs, batch_length, bs, 
  ...
  for epoch in range(start_epoch, n_epochs):
    with ExitStack() if not timer else Timer("Filter"):
      with torch.no_grad():
        X, X_track, X_intermediate, train_neg_log_likelihood = da_methods.EnKF(ode_func, obs_func, t_obs, batch_y_obs, N_ensem, init_m, init_C_param, model_Q_param, noise_R_param, device, t0=t0, **enkf_kwargs) # (n_obs, bs*n_train, N_ensem, x_dim)
      if m_step_type == "mean":
  ...
  for epoch in range(start_epoch, n_epochs):
    ...
    with ExitStack() if not timer else Timer("Filter"):
      ...
      with torch.no_grad():
        ...
        if method == 'enkf':
          X, X_track, X_intermediate, neg_log_likelihood = da_methods.EnKF(ode_func, cur_obs_func, t_obs[:likelihood_warmup], batch_y_obs[:likelihood_warmup], N_ensem, init_m, init_C_param, model_Q_param, noise_R_param, device, detach_every=detach_every, t0=t0, **enkf_kwargs)
        elif method == 'kf':
          m_track, C_track, _, neg_log_likelihood = da_methods.KF(ode_func, cur_obs_func, t_obs[:likelihood_warmup], batch_y_obs[:likelihood_warmup], init_m, init_C_param, model_Q_param, noise_R_param, device, **enkf_kwargs)
        elif method == 'pf':
          X, w, X_track, X_intermediate, w_track, w_intermediate, neg_log_likelihood = da_methods.BootstrapPF(ode_func, cur_obs_func, t_obs[:likelihood_warmup], batch_y_obs[:likelihood_warmup], N_ensem, init_m, init_C_param, model_Q_param, noise_R_param, device, proposal, adaptive_resampling, detach_every=detach_every, t0=t0, **enkf_kwargs)
      ...
      for start in range(likelihood_warmup, n_obs, tbptt):
        ...
        if method == 'enkf':
          X, X_track, X_intermediate, neg_log_likelihood = da_methods.EnKF(ode_func, cur_obs_func, t_obs[start:end], batch_y_obs[start:end], N_ensem, init_m, init_C_param, model_Q_param, noise_R_param, device, detach_every=detach_every, t0=t_start, init_X=X, **enkf_kwargs)
          X = X.detach()
        elif method == 'kf':
          m_track, C_track, _, neg_log_likelihood = da_methods.KF(ode_func, cur_obs_func, t_obs[start:end], batch_y_obs[start:end], init_m, init_C_param, model_Q_param, noise_R_param, device, **enkf_kwargs)
        elif method == 'pf':
...
def test_epoch(ode_func, obs_func, t_obs, y_obs_test, out_test, init_m, init_C_param, model_Q_param, noise_R_param, device, proposal, adaptive_resampling, detach_every, test_filter,
  ...
  with torch.no_grad():
    while True:
      ...
      if test_filter:
        if method_test == 'enkf':
          _, X_track_test, _, test_neg_log_likelihood = da_methods.EnKF(ode_func, obs_func, t_obs, y_obs_test, N_ensem_test, init_m, init_C_param, model_Q_param, noise_R_param, device, detach_every=detach_every, **enkf_kwargs_copy)
          if true_system == "L63": 
          ...
          _, _, X_track_test, _, w_track_test, _, test_neg_log_likelihood = da_methods.BootstrapPF(ode_func, obs_func, t_obs, y_obs_test, N_ensem_test, init_m, init_C_param, model_Q_param, noise_R_param, device, proposal, adaptive_resampling, detach_every=detach_every, **enkf_kwargs_copy)
          if true_system == "L63": 
            test_mse1 = torch.sqrt(
              utils.particle_mse_loss_last_dim(X_track_test[burn_in:], out_test[burn_in:], w_track_test[burn_in:]))
            test_mse2 = torch.sqrt(
              utils.particle_mse_loss(X_track_test[burn_in:], out_test[burn_in:], w_track_test[burn_in:])).unsqueeze(0)
            test_mse = torch.cat((test_mse1, test_mse2))
          elif true_system == "L96":
            test_mse = torch.sqrt(
              utils.particle_mse_loss(X_track_test[burn_in:], out_test[burn_in:], w_track_test[burn_in:]))
...
def print_epoch(ode_func, obs_func, t_obs, y_obs_test, out_test, init_m, init_C_param, model_Q_param, noise_R_param, device, 
  ...
  with torch.no_grad():
    if true_system == "L63":
      ...
      out_test1, _, _, _ = generate_data.generate_data(ode_func, None, t_obs_test, u0_test[rdn], None, None, device, ode_method='rk4', ode_options=dict(step_size=0.01), adjoint=False, save_intermediate_step=False)
      out_test_true1, _, _, _ = generate_data.generate_data(true_ode_func, None, t_obs_test, u0_test[rdn], None, None, device, ode_method='rk4', ode_options=dict(step_size=0.01), adjoint=False, save_intermediate_step=False)
...
#           out_test1, _, _, _ = generate_data.generate(ode_func, None, t_obs_test, first_out[rdn,0], None, None, device, ode_method='rk4', ode_options=dict(step_size=0.05),adjoint=False, save_intermediate_step=False)
#           out_test_true1, _, _, _ = generate_data.generate(true_ode_func, None, t_obs_test, first_out[rdn,0], None, None, device, ode_method='rk4', ode_options=dict(step_size=0.05),adjoint=False, save_intermediate_step=False)
